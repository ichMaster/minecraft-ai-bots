...
Редактор не показує змін усередині `mindcraft/`, хоча деплой їх там робить: клон має власний git, і щоб його зміни не виглядали як незакомічена робота, `deploy.sh` додає згенероване в його `.gitignore`, а відстежуваним файлам ставить `skip-worktree`. Плюс `.vscode/settings.json` велить редактору взагалі не відкривати цей вкладений репозиторій.
