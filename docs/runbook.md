...
Так можна перезапускати одного бота, не гасячи решту, і тримати логи роздільно. Розплата: у кожного свій mindserver, тому бот не бачить інших і вважає, що він на сервері сам — і всі троє відповідатимуть на кожне повідомлення в загальному чаті. Через `/msg` поводяться так само, як і при спільному запуску.
...
Результати записати в `docs/test-log.md`. Порахувати запити до Gemini:
